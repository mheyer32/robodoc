...
/*
 * ASCII AMIGAGUIDE HTML DBSGML LATEX RTF 
 *
 * DCD Added DBSGML 
 */
...
char *att_start_command[SIZE_ATTRIBUTES][SIZE_MODES] = {
    /* Large Font */
    {"", "@{b}", "<FONT SIZE=\"+1\">", "<sect2><title>", "{\\large ", "\\par\\fs28 ", "\\fB "},
    /* Italics. */
    {"", "@{i}", "<I>", "<emphasis>", "{\\it ", "\\i1 ", "\\fI "},
    /* NON-Proportional font. */
    {"", "", "", "", "", "", ""},
    /* Small Font. */
    {"", "", "<SMALL>", "<bridgehead renderas=\"sect4\">", "{\\small ", "\\fs16 ", "\\fI"}, 
    /* Bold.*/
    { "", "@{b}", "<B>", "<emphasis role=\"strong\">", "{\\bf ", "\\b1 ", "\\fB "},
    /* Underline */
    {"", "@{u}", "<U>", "<![CDATA[", "\\underline{", "\\ul1 ", ""}, 
    /* Shine */
    {"", "@{fg shine}", "<EM>", "<emphasis role=\"strong\">", "{\\em ", "", "\\fR"},
    /* Highlight */
    { "", "@{fg highlight}", "<EM>", "<emphasis>", "{\\em ", "", "\\fB "} 
};

char           *att_stop_command[SIZE_ATTRIBUTES][SIZE_MODES] = {
    /* Large Font */
    {"", "@{ub}", "</FONT>", "</title>", "}", "\\fs20\\line ", "\\fP "}, 
    /* Italics. */
    {"", "@{ui}", "</I>", "</emphasis>", "}", "\\i0 ", "\\fP "}, 
    /* NON-Proportional font. */
    {"", "", "", "", "", ""},
    /* Small Font. This will be bold in SGML */
    {"", "", "</SMALL>", "</bridgehead>", "}", "\\fs20 ", "\\fP "},
    /* Bold.  */
    {"", "@{ub}", "</B>", "</emphasis>", "}", "\\b0 ", "\\fP "}, 
    /* Underline */
    {"", "@{uu}", "</U>", "]]>", "}", "\\ul0 ", "\\fP "},
    /* Shine */
    {"", "@{fg text}", "</EM>", "</emphasis>", "}", "", "\\fP "},
    /* Highlight */
    {"", "@{fg text}", "</EM>", "</emphasis>", "}", "", "\\fP "} 
...
int RB_Get_Item_Type(char *cmp_name)
{
    ...
    for (item_type = NAME_ITEM; item_type < OTHER_ITEM; ++item_type) {
        if (!strncmp(item_names[item_type], cmp_name,
                     strlen(item_names[item_type]))) return (item_type);
    }
    return (NO_ITEM);
}
...
int RB_Get_Item_Attr(char *cmp_name)
{
    ...
    if (strcmp("NORMAL", cmp_name)) {
        fprintf(stderr,
                "%s: Warning unknown attribute [%s] in defaults file.\n",
                whoami, cmp_name);
    }
    return (MAKE_NORMAL);
}
