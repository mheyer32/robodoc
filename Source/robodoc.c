 ...
 *   There are six different file output formats:
 *       ASCII
 ...
 *       DBSGML (Docbook SGML) -- this technical documentation standard format is used by
 *             many technical writers (including the Linux Documentation Project).
 *             It allows the use of stylesheets and can be converted to
 *             a host of different formats including rtf, html, latex,
 *             pdf, and text. See www.docbook.org and www.linuxdoc.org
 *             for more details.
 ...
 *
 *  David Druffner: <ddruff@gemini1consulting.com>
 *     Addition of DBSGML support and -rh switch
 *
 * CREATION DATE
 *   20-Dec-94
 ...
 *        allowing standardized conversion to a host of formats including HTML
 *        (single and multiple), RTF, ASCII, LATEX, and PDF. Docbook also 
 ...
 *
 *   Modification by Frans Slothouber
 *      March 2002
 ...
 *   Modification by Frans Slothouber
 *      May 2002
 *      o Modified the RB_Find_End_Marker routine so that it
 *        dynamically enlarges a buffer with the header text while
 *        it is read.  This way fseek and ftell are no longer needed
 *        and the code should work much better on Win32 machines.
 * NOTES
 ...
 *     Under IRIX
 *     On a Linux box with gcc, Slackware, Redhat, Mandrake 7.2, and Debian 2.1.
 * BUGS
...
#ifdef _AMIGA
char            RB_VER[] =
    "\0$VER: robodoc " VERSION " " __AMIGADATE__
    " (c) by Maverick Software Development 1994-2001";
#else
char            RB_VER[] =
...
char            use[] =
    "ROBODoc Version " VERSION "( w/DBSGML support as modified by DCD),\n"
    " autodocs formatter ($Revision: 1.14 $)\n"
    "(c) 1994-2002 Frans Slothouber and Jacco van Weert\n"
    ...
    "  -rh             - Use the distinct /*ROBODOC* header and *ROBODOC_END* end marker instead of \n"
    "                    generic markers\n"
    "  -nt              -No table - in DBSGML mode, causes master index items to be listed as distinct\n"
    "                    section 3 items rather than put in a table\n\n"
    "The type of output is selected with one of the following switches:\n"
    "  ASCII, GUIDE, HTML, DBSGML, LATEX, TROFF, or RTF\n"
    ...
    " Jacco van Weert,\n"
    "  Petteri Kettunen, Bernd Koesling, Thomas Aglassinger, Anthon Pang, Stefan Kost,\n"
    "  David Druffner, Sasha Vasko.\n"
        ...
        if ((source = fopen(argv[1], "r")) != NULL) {
            if (!(course_of_action & DO_INDEX)) {
                ...
                if ((course_of_action & DO_USE_XREFS) && file_with_xrefs) {
                    if ((xreffiles_file = fopen(file_with_xrefs, "r")) !=
                        ...
                        RB_Panic
                            ("can't open file with xref files \"%s\"\n",
                             file_with_xrefs);
                    }
                }
            } else { /* INDEX */
            ...
            if (course_of_action & DO_MAKE_DOCUMENT) {
                ...
                RB_Say("trying to open xref destination file \"%s\"\n",
                        output_file_for_xrefs);
                if ((documentation = fopen(output_file_for_xrefs, "w")) !=
                        NULL) {
...
void RB_Analyse_Arguments(int argc, char **argv,
        char **file_with_xrefs, char **output_file_for_xrefs)
{
    char          **parameter;
    int             parameter_nr;

    for (parameter_nr = argc - 3, parameter = argv + 3;
            parameter_nr > 0; parameter++, parameter_nr--) {
        ...
        if (!RB_Str_Case_Cmp(*parameter, "HTML"))
            ...
            output_mode = RTF;
        else if (!RB_Str_Case_Cmp (*parameter, "TROFF"))
            output_mode = TROFF;
        else if (!RB_Str_Case_Cmp(*parameter, "DBSGML"))
            output_mode = DBSGML;
            ...
            if (!RB_Str_Case_Cmp(*parameter, "DBSGML")) {
             /* don't print footer to master index for DBSGML 
              * since going to concatenate files */
                course_of_action &= DO_SINGLEDOC;
            }
        } else if (!RB_Str_Case_Cmp(*parameter, "-RH"))
          /* If this is set, will only recognize *ROBOHEAD* as a header */
            course_of_action |= DO_ROBO_HEAD;
        else if (!RB_Str_Case_Cmp(*parameter, "-NT"))
          /* If this is set, will only recognize *ROBOHEAD* as a header */
            course_of_action |= DO_NO_TABLE;
            ...
            if (--parameter_nr) {
                ...
                RB_Panic
                    ("you must specify a title with the TITLE option\n");
            ...
            if (--parameter_nr) {
                ...
                *file_with_xrefs = *parameter;
                ...
                RB_Panic
                    ("you must specify a xref file with the XREF option\n");
            ...
            if (--parameter_nr) {
                ...
                *output_file_for_xrefs = *parameter;
                ...
                RB_Panic
                    ("you must specify a xref file with the GENXREF option\n");
        } else {
            RB_Panic("unknown option %s\n", *parameter);
        }
    }
    if ((course_of_action & DO_USE_XREFS) &&
            (output_mode == ASCII) && !(course_of_action & DO_INDEX)) {
    ...
    if (course_of_action & DO_INDEX) {
        if ((output_mode != LATEX) && (output_mode != HTML)
                && (output_mode != DBSGML)) {
            RB_Panic
                ("you can only use the INDEX option in combination with LATEX, HTML, or DBSGML\n");
        }
    }
}
...
void RB_Close_The_Shop(void)
{
    ...
    *tmp_header;
    ...
    if (documentation) { fclose(documentation);}
